...
SELECT
	...
	CASE
		WHEN IdleSDate = NULL
		THEN NULL
		ELSE DATEDIFF(month, IdleSDate, '2022-04-25' )
	END AS IdleMonths
FROM
	(
--Getting the status for the last 13 months
		SELECT
			...
			YearMfd,
			CompEqCode,
			TRIM(ISNULL([2021-03-25],'')) AS Mar,
			TRIM(ISNULL([2021-04-25],'')) AS Apr,
			TRIM(ISNULL([2021-05-25],'')) AS May,
			TRIM(ISNULL([2021-06-25],'')) AS Jun,
			TRIM(ISNULL([2021-07-25],'')) AS Jul,
			TRIM(ISNULL([2021-08-25],'')) AS Aug,
			...
			TRIM(ISNULL([2022-03-25],'')) AS MarC
		FROM
			(
--Getting the equipment description
				...
				WHERE
					dbo.tblOpStatus.RD BETWEEN '2021-03-25' AND '2022-03-25' AND
					dbo.tblM6a.rMonth = 3 AND
